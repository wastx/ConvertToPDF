...
class ImageToPDFConverter:
    def __init__(self, root):
        self.root = root
        self.image_paths = []
        self.out_pdf_name = tk.StringVar()
    ...
    def initialize_ui(self):
        ...
        pdf_name_entry = tk.Entry(self.root, textvariable=self.out_pdf_name, width=40, justify='center')
        pdf_name_entry.pack()
    ...
    def convert_images_to_pdf(self):
        if not self.image_paths:
            return
        output_pdf_path = self.out_pdf_name.get() + ".pdf" if self.out_pdf_name.get() else "output.pdf"
